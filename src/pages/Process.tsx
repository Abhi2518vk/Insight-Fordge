      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-16">
        {currentSteps.map((s) => (
          <button
            ...
            className={`p-6 rounded-xl border text-center transition-all duration-300 cursor-pointer flex flex-col items-center gap-3 ${
              currentStepNum === s.num
                ? 'bg-bg-surface border-brand-primary text-text-primary shadow-[0_4px_25px_rgba(79,70,229,0.15)] scale-[1.02] ring-1 ring-brand-primary/30'
                : 'bg-bg-surface/30 border-border-custom text-text-secondary hover:text-text-primary hover:border-border-custom/80'
            }`}
          >
            <span className={`font-mono text-[9px] uppercase tracking-wider font-bold ${currentStepNum === s.num ? 'text-accent-gold' : 'text-text-secondary'}`}>Phase 0{s.num}</span>
            <div className={`p-3 rounded-full border ${currentStepNum === s.num ? 'border-brand-primary text-brand-primary bg-bg-dark' : 'border-border-custom bg-bg-dark/20 text-text-secondary'}`}>
              {s.icon}
            </div>
            <span className="font-display text-xs font-semibold hidden sm:inline truncate max-w-full text-ellipsis">{s.title}</span>
          </button>
        ))}
      </div>

      {/* 3. Render Step Detail Panel */}
      <CardGlass className="p-10 sm:p-14 border-brand-primary/30 animate-forge shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className="flex items-center gap-2 font-mono text-[10px] text-accent-gold uppercase tracking-wider bg-bg-dark border border-border-custom px-3.5 py-1.5 rounded-md w-fit font-bold">
              <span>Phase 0{currentStep.num}: {currentStep.tag}</span>
            </div>

            <h2 className="font-display text-3xl sm:text-4xl font-extrabold text-text-primary tracking-tight leading-tight">{currentStep.title}</h2>
            <p className="text-text-secondary text-base sm:text-lg leading-relaxed">{currentStep.desc}</p>
          </div>
